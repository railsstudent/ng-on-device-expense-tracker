...
export class ModelDownloaderService {
  ...
  readonly isOnline = createOnlineStatusSignal(this.#navigator);

  // Directly reference already-read-only signals to simplify the reactive graph
  readonly status = this.#cacheService.status;
  readonly progress = this.#cacheService.progress;

  // Derive connection/caching states
  readonly isCached = computed(() => this.status() === 'cached');
  readonly isDownloading = computed(() => this.status() === 'downloading');

  // Disable button if offline and not already downloaded/cached
  readonly isDownloadDisabled = computed(() => !this.isOnline() && !this.isCached());
