...
{
  "isReceipt": true if the provided text represents a valid cash receipt, transaction invoice, or bill. Otherwise, false (boolean)",
  ...
  "category": "One of these specific values: 'Food', 'Groceries', 'Transport', 'Entertainment', 'Shopping', 'Utilities', 'Medical', or 'Others' (string). If unknown, use 'Others'"
}

RULES:
- Do NOT guess values. If a value is missing or unreadable, strictly use its specified unknown/default fallback value.
- If the OCR text is clearly not a receipt, or is completely empty or unreadable, set "isReceipt" to false, "merchantName" to "Unknown Merchant", "amount" to 0.00, "transactionDate" to "${currentDateString}", and "category" to "Others".
