...
import { HistoryInsightsService } from '@/features/history-insights/components/history-insights/services/history-insights.service';
...
describe('HistoryInsightsService', () => {
  ...
  describe('Async Database Operations (injectAsync)', () => {
    it('should asynchronously resolve DatabaseService and query expenses by date range', async () => {
    ...
    it('should asynchronously resolve DatabaseService and delete record by id', async () => {
      await service.deleteExpense(42);
      expect(mockDbService.delete).toHaveBeenCalledWith(42);
    });

    it('should handle sequential async operations across multiple calls', async () => {
      await service.loadExpenses('2026-08-01', '2026-08-15');
      await service.deleteExpense(1);
      expect(mockDbService.selectByDateRange).toHaveBeenCalledTimes(1);
      expect(mockDbService.delete).toHaveBeenCalledWith(1);
    });
  });
