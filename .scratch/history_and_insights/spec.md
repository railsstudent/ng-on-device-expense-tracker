...
2. **Responsive Date Search**: As a user, I want to input an inclusive date range (From/To) spanning a maximum of 31 days and click "Search" to fetch transactions matching my criteria instantly. If the range is invalid or exceeds 31 days, I want to see an immediate warning banner.
...
- **Ergonomic Desktop Split Layout**: To maximize horizontal data grid legibility on desktop displays, the workspace grid uses an 8 / 4 column split configuration inside `.split-container` (8/12 grid columns for the ledger on the left, 4/12 grid columns for the AI chat sidebar on the right). This ensures expansive space for columns like Merchant and Category without sacrificing a beautiful, vertical, sidebar-like chat ergonomic flow.
- **Mobile-Only Dedicated Sorting Selector**: Since horizontal table headers are hidden under `@container` card layout configurations on small screen wrappers, a dedicated `<app-history-mobile-sort>` subcomponent must render at the top of the container. It queries the `table-container` width and becomes visible only when the container is narrow. This component translates dropdown change selections directly into a standard output event containing the `TableSortState` object to update the parent state reactively without duplicating sorting algorithms.
- **Date-Range UI Validation Guardrail**: To guarantee optimal client-side performance and prevent WebGPU memory or context-exhaustion crashes, date searches are strictly limited to a **maximum span of 31 days**. The `HistorySearchFormComponent` must validate this range in `onSubmit`. If `endDate` is prior to `startDate` or the day difference exceeds 31 days, a localized error signal is set, a premium warning banner is displayed inside the form, and the submit event is blocked.
...
- **Priming phase**: When a new search is submitted, the previous session is destroyed. A new `Conversation` is created under a safe 4,096 token Engine limit, and a compact pipe-separated CSV dataset (`Date|Category|Merchant|Amount`) along with the precomputed stats JSON is sent. Gemma 4 caches the attention weights in memory and confirmation is returned.
- **Turn-Based Auto-Reset Guardrail**: Over multi-turn conversations, historical prompts accumulate and risk overflowing Gemma's token limits. To ensure absolute crash-free stability, the system tracks active user turns. After 3 turns, the active chat session is automatically destroyed, deleted from memory, and re-primed silently with the current dataset before sending the next query.
- **Sliding 2-Question Memory Array**: To prevent complete context memory loss during turn resets, the system maintains a private client-side list of the last 2 user questions. During re-priming, these queries are formatted and injected as a brief semantic thread hint to support natural follow-up conversation flow.
