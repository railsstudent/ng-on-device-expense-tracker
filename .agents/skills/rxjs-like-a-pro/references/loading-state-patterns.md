...
type LoadingState<T> = { loading: true } | { loading: false; data: T } | { loading: false; error: unknown };

function withLoadingState<T, R>(project: (value: T) => Observable<R>): OperatorFunction<T, LoadingState<R>> {
  return (source) =>
    source.pipe(
      switchMap((value) =>
        project(value).pipe(
