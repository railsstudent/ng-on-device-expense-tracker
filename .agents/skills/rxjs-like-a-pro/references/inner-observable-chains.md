...
click$.pipe(switchMap(() => concat(of('show'), timer(3000).pipe(map(() => 'hide'))))).subscribe((action) => {
...
notification$.pipe(switchMap((message) => showNotification(message))).subscribe((state) => render(state));
```
